...
def parse_arguments():
    ...
    parser.add_argument(
        '--input_text',
        type=str,
        default='续写：RTX4090具有760亿个晶体管，16384个CUDA核心')
    parser.add_argument(
        '--input_tokens',
        type=str,
...
if __name__ == '__main__':
    ...
    for i in range(len(output_ids.tolist())):
        ...
        end_tuili=time.time()
        timeconsume=end_tuili - start_tuili
        average=timeconsume/len("".join(output_text))
        print("函数运行时间：",average)
        print("函数运行时间：",timeconsume)
